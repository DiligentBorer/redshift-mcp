...
1. 环境变量 `REDSHIFT_MCP_ERROR_API_CONFIG` 指向的文件（不重新 build wheel 就想换配置时用）；
...
**未配置的行为**：启动时 `error_api` 抛错并被 `load_plugins` 隔离跳过（不搞崩 server），日志给出
...
cp src/redshift_mcp_error_api/config.example.yaml src/redshift_mcp_error_api/config.yaml
cp src/redshift_mcp_error_api/queries/error_api.example.sql src/redshift_mcp_error_api/queries/error_api.sql
```
...
```bash
uv build --package redshift-mcp-error-api          # 真实配置随 wheel；模板被排除
uv pip install dist/redshift_mcp_error_api-*.whl    # 装进与主程序同一个 venv
```
